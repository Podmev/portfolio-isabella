...
function getWriterName(portfolio) {
  return portfolio?.profile?.publicName || portfolio?.user?.name || portfolio?.writer?.profile?.publicName || "the writer";
}

export default function CopyVortexProfileCta({ portfolio, locale = "en" }) {
    ...
    <section className="border-t border-border bg-surface-alt px-6 py-10 md:py-12">
      <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-5 rounded-[24px] border border-border bg-card p-6 shadow-sm sm:flex-row sm:items-center md:p-8">
        <div className="max-w-2xl">
          <p className="text-[11px] font-medium uppercase tracking-[0.24em] text-muted-foreground">Copy Vortex profile</p>
          <h2 className="mt-2 text-2xl leading-tight md:text-3xl">See the detailed writer page</h2>
          <p className="mt-3 text-sm leading-6 text-muted-foreground md:text-base">
            Open {name}&apos;s full Copy Vortex profile for more public works, niches, companies, projects, and portfolio details.
          </p>
        </div>
