...
function formatPeriod(item, t) {
  const start = item.startDate || "";
  const end = item.isCurrent ? t("experiencePresent") : item.endDate || "";
...
export default function ExperienceSection({ portfolio }) {
  ...
  const name = portfolio?.profile?.publicName || portfolio?.user?.name || t("writerFallbackName");

  if (!items.length) return null;

  return (
    <Section className="section-soft-gradient py-14 md:py-16">
      <SectionTitle eyebrow={t("experienceEyebrow")} title={t("experienceTitle")} subtitle={t("experienceSubtitle")} />

      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
        <div className="divide-y divide-border/70">
          {items.map((item) => (
            <div key={item.id || `${item.title}-${item.companyName}`} className="py-5 first:pt-0 last:pb-0">
              <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{formatPeriod(item, t) || item.location}</p>
              <h3 className="mt-2 text-lg leading-snug md:text-xl">
                {item.title}{" "}
                {item.companyName ? <span className="text-muted-foreground">{t("experienceAt")} {item.companyName}</span> : null}
              </h3>
        ...
        <div className="mx-auto w-full max-w-sm lg:max-w-md">
          <PortfolioPortrait src={image} alt={t("portraitAlt", { name })} />
        </div>
      </div>
    </Section>
  );
